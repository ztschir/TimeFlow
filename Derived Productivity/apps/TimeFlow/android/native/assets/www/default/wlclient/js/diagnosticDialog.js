...
__WLDiagnosticDialog = function() {
	this.showDialog = function(title, messageText, allowReload, allowDetails, response) {
			var buttons = [];
			if (allowReload) {
				buttons.push({
				...
				buttons.push({
					...
					handler : function() {
						WL.Device.getNetworkInfo(function(networkInfoObject) {
							showDiagnosticTable(response, networkInfoObject);
						});
					}
				});
			}
		...
		function showDiagnosticTable(response, networkInfo) {
			// Back again with networkInfo object
			var diagnosticData = getDiagnosticData(response, networkInfo);
		...
		function getDiagnosticData(response, networkInfo) {
			var diagnosticData = {};
			...
			diagnosticData[WL.ClientMessages.diagScreenResolution] = (WL.App.getScreenWidth ? WL.App.getScreenWidth() : screen.width) + 'x' + (WL.App.getScreenHeight ? WL.App.getScreenHeight() : screen.height);
			if (typeof networkInfo != "undefined") {
				diagnosticData[WL.ClientMessages.diagAirplaneMode] = networkInfo.isAirplaneMode == null ? WL.ClientMessages.notAvailable : networkInfo.isAirplaneMode;
				diagnosticData[WL.ClientMessages.diagUsingNetwork] = networkInfo.networkConnectionType;
				diagnosticData[WL.ClientMessages.diagWifiName] = networkInfo.wifiName == null ? WL.ClientMessages.notAvailable : networkInfo.wifiName;
				diagnosticData[WL.ClientMessages.diagMobileNetworkType] = networkInfo.telephonyNetworkType == null ? WL.ClientMessages.notAvailable : networkInfo.telephonyNetworkType;
				diagnosticData[WL.ClientMessages.diagCarrierName] = networkInfo.carrierName == null ? WL.ClientMessages.notAvailable : networkInfo.carrierName;
			...
			diagnosticData[WL.ClientMessages.diagErrorMessage] = response.errorMsg;
		...
		function formatDiagnosticData(diagnosticData) {
			...
			for (property in diagnosticData) {
				var diagnosticDataValue = (typeof diagnosticData[property] == "undefined" || diagnosticData[property] == "undefined") ? "" : diagnosticData[property];
