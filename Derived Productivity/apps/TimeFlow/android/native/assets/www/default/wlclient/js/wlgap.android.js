...
WL.App.update = function() {
    cordova.exec(null, null, "WebResourcesDownloader", "updateApp", [WL.NativePage._getCookiesForNative(), WL._InstanceId.value]);
};
...
__WLNativePage = function() {
     ...
     *                invoked.
     * @param data (optional) {object} - a JSON object that will be sent to the native class. The data must be
     *                single dimensioned
     */
    ...
    /**
     * Internal use. create a text representation of the session cookies, for use of native Android code when
     * sending requests for the worklight server
     */
...
__WLTabBarItem = function() {
     ...
     * unaffected through calls to WLTabBar.setEnabled.
...
__WLTabBar = function() {
    ...
    /**
     * Initializes the TabBar. Must be called before using any other TabBar function. If you use
     * setParentDivId, call setParentDivId before init.
     */
    this.init = function() {
	if (isInitialized()) {
	    this.removeAllItems();
    ...
    /**
     * In case you need the tab bar will be added to other div out of content, Call this function before
     * init() with the parentDivId
     ...
     * @param {Object} [options] Options for customizing the individual tab item imgSrc - imgSrc for normal
     *                tab imgSrcSelected - imgSrc for selected tab
     * 
	...
	// Workaround: Bug 3389 - [Android] If content div is shown after auth div, a space is added to the
...
__WLOptionsMenu = function() {
    ...
    /**
     * Initializes the Android toolbar, enabling it, and making it visible. Must be called before using it.
     * 
    ...
    /**
     * Adds an item to the Android Options Menu. Can be called only after initializing the menu. Items are
     * ordered on the menu according to the order in which they were added. If you add a item with an existing
     * ID, the new item replaces the existing one.
     * 
     ...
     * 
     * @param options - Hash options object. Available options: image - The path to the item's icon, starting
     *                from the application root directory. Per Android's guidelines, the icon should be a
     *                48-by-48 pixel black and white PNG file. enabled - Boolean. Defines whether the item is
     *                enabled or disabled.
     * @return Item
     */
    this.addItem = function(id, callback, title, options) {
	...
	var itemToReturn = NativeOptionsMenu.addItem(id, "WL.OptionsMenu.__onItemClicked('" + id + "')", title, defaultOptions.image, defaultOptions.enabled);
	this.__addWLSettingItem();
	return itemToReturn;
    };
	...
	return NativeOptionsMenu.addItem(SETTING_OPTIONS_MENU_ITEM, "WL.OptionsMenu.__onItemClicked('wlSettings')", "Worklight Settings", "settings", true);
    };
...
__WLPush = function() {
	...
	WL.Validators.validateArguments(['string', 'string', 'string', WL.Validators.validateFunctionOrNull], arguments, 'WL.Client.Push.registerEventSourceCallback');
	...
	cordova.exec(function(deviceToken) {
	    ...
	    WL.SimpleDialog.show(WL.ClientMessages.error, WL.ClientMessages.notificationUpdateFailure, [{text:  WL.ClientMessages.ok}]);
	...
	// isRegistering means If check from register function, then registeredEventSources not exist yet.
...
WL.App.__showWLPreferences = function() {
	WL.NativePage.show("com.worklight.common.WLPreferences", function(data) {
    	...
    	WL.Utils.__getSkinFromRemoteSkinLoader(newAppId, newAppVersion, function(skinName){
    		...
    		cordova.exec(null, null, "WebResourcesDownloader", "switchApp", [WL.NativePage._getCookiesForNative(), WL._InstanceId.value, newAppId, newAppVersion]);
...
WL.DeviceAuth.__init = function(deviceProvisioningSettings, successCallback, failureCallback) {
    cordova.exec(successCallback, failureCallback, "DeviceAuth", "init", [deviceProvisioningSettings]);
};
